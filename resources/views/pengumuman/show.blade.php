            ...
            <div class="col-md-8">
                <div class="card shadow mb-4">
                    @if ($pengumuman->thumbnail)
                        <img src="{{ asset('storage/' . $pengumuman->thumbnail) }}" class="card-img-top"
                        ...
                        <div class="mb-2 text-muted mt-5">
                            ...
                            @if ($pengumuman->efficient_start_date || $pengumuman->efficient_end_date)
                                ...
                                @if ($pengumuman->efficient_start_date)
                                    {{ \Carbon\Carbon::parse($pengumuman->efficient_start_date)->format('d-m-Y') }}
                                @endif
                                @if ($pengumuman->efficient_end_date)
                                    Hingga {{ \Carbon\Carbon::parse($pengumuman->efficient_end_date)->format('d-m-Y') }}
                                @endif
                            @endif
                        </div>
                        <hr class="my-3">
                ...
                <a href="{{ route('pengumuman.index') }}" class="btn btn-outline-primary"><i class="fas fa-arrow-left"></i>
                    Kembali ke Daftar Pengumuman</a>
            </div>
        </div>
    </div>
@endsection
